...
type kubeGenericRuntimeManager struct {
	...
	// Callback invoked after pod sandbox is ready. See function SetPodSandboxReadyCallback.
	onPodSandboxReady func(context.Context, *v1.Pod) error
...
func (m *kubeGenericRuntimeManager) SyncPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff) (result kubecontainer.PodSyncResult) {
	...
	if podContainerChanges.CreateSandbox {
		...
		// At this point, volumes are already mounted before SyncPod at the kubelet (SyncPod) level, so,
		// all three requirements - sandbox + network + volumes are met to update `PodReadyToStartContaniers` contition.
		if m.onPodSandboxReady != nil {
			klog.V(3).InfoS("Pod sandbox and network ready, invoking callback", "pod", klog.KObj(pod), "podIPs", podIPs)
			if err := m.onPodSandboxReady(ctx, pod); err != nil {
				// Log the error but don't fail pod creation, in order to retain the existing behaviour
				klog.V(2).InfoS("Failed to invoke sandbox ready callback, continuing with pod creation", "pod", klog.KObj(pod), "err", err)
			}
		}
	}
...
// SetPodSandboxReadyCallback registers a callback to be invoked after pod sandbox creation during
// (containerRuntime) SyncPod. The callback is invoked before image pulling begins, allowing kubelet to
// update PodReadyToStartContaniers condition correctly, at the right time.
func (m *kubeGenericRuntimeManager) SetPodSandboxReadyCallback(callback func(context.Context, *v1.Pod) error) {
	klog.V(2).InfoS("Registering pod sandbox ready callback for PodReadyToStartContainers condition")
	m.onPodSandboxReady = callback
}
